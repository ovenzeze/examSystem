	...
	$('form :input').blur(function(){
		...
		$('#addUserInform').click(function () {
			//匹配学号和用户名
			 	...
			 	//添加至下一行
                $('table tr:last').after($('<tr><td class="checkbox m-0"><label><input type="checkbox" name="a"></label></td><td></td><td></td><td></td><td><a href="#" id="">修改</a>/<a href="#" class="deleteOne">删除</a></td></tr>'));
...
function changeInfo(){
	...
	$('form :input').blur(function(){
		...
		$('#addUserInform').click(number,function(event){
			...
			$('#userTbody tr:eq('+event.data+') td:eq(3)').text($('#inputCourse').val());
			$('#zz').css('display','none');
		    $('#addUser').css('display','none');
		});
